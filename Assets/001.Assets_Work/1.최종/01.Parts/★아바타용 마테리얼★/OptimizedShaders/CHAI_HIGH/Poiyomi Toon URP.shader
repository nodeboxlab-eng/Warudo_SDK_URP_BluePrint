Shader "Hidden/Locked/.poiyomi/Poiyomi Toon URP/44a251282c56a3f4e94ba909e8ea975f"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float4 frag(VertexOut i, bool facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, pow(abs(poiFragData.baseColor), 0.9), hueShiftAlpha.a);
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), - (-0.58) * hueShiftAlpha.b);
				if (0.0 != 0)
				{
					#if !defined(UNITY_COLORSPACE_GAMMA)
				...
				poiFragData.baseColor = saturate(lerp(poiFragData.baseColor, poiFragData.baseColor * (-0.17 + 1), hueShiftAlpha.g));
				#endif
				if (0.0)
				{
