...
VertexOutput vert(VertexInput v)
{
    ...
    o.positionWS = TransformObjectToWorld(outlinePos);
...
half4 frag(VertexOutput i) : SV_TARGET
{
    ...
    if (_SurfaceType >= TRANSPARENT_SURFACE)
        clip(-1);

    const float clippingMask = SelectMask(SAMPLE_TEXTURE2D(_ClippingMask, sampler_MainTex, TRANSFORM_TEX(i.uv0, _ClippingMask)), _ClippingMaskCH);
    ...
    {
        half4 baseMap = SAMPLE_TEXTURE2D_LOD(_MainTex, sampler_MainTex, TRANSFORM_TEX(i.uv0, _MainTex), 0);
    ...
    half alpha = OutputAlpha(outlineAlpha, false);
#if _ALPHATEST_ON
